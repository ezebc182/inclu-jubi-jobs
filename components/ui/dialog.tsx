...
const dialogVariants = cva("bg-background text-foreground shadow-lg rounded-lg w-full max-w-md", {
  variants: {
    variant: {
      default: "",
...
>(({ className, children, ...props }, ref) => (
  <RadixDialogContent ref={ref} className={cn(dialogVariants(), className)} {...props}>
    {children}
  </RadixDialogContent>
))
...
const DialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("sm:flex justify-end gap-2 p-6", className)} {...props} />
)
...
>(({ className, ...props }, ref) => (
  <RadixDialogTitle ref={ref} className={cn("text-lg font-semibold leading-none tracking-tight", className)} {...props} />
))
...
>(({ className, ...props }, ref) => (
  <RadixDialogDescription ref={ref} className={cn("text-sm text-muted-foreground", className)} {...props} />
))
...
const DialogTabsList = forwardRef<React.ElementRef<typeof Slot>, React.ComponentPropsWithoutRef<typeof Slot>>(
  ({ className, ...props }, ref) => (
    <Slot
      ref={ref}
      className={cn("inline-flex items-center justify-center rounded-md p-1 text-foreground", className)}
      {...props}
    />
  ),
)
...
const DialogTabsTrigger = forwardRef<React.ElementRef<typeof Slot>, React.ComponentPropsWithoutRef<typeof Slot>>(
  ({ className, ...props }, ref) => (
    <Slot
      ref={ref}
      className={cn(
...
const DialogTabsContent = forwardRef<React.ElementRef<typeof Slot>, React.ComponentPropsWithoutRef<typeof Slot>>(
  ({ className, ...props }, ref) => (
    <Slot
      ref={ref}
      className={cn(
